...
resource "harness_platform_triggers" "crontrigger" {
  org_id        = var.org_id
  project_id    = var.project_id
  name          = var.refcrontrigger
  identifier    = var.refcrontrigger
  target_id     = var.refpipeline
  yaml          = templatefile("crontrigger.yaml", {
    org_identifier = var.org_id
    project_identifier = var.project_id
    trigger_name = var.refcrontrigger
    trigger_identifier = var.refcrontrigger
    inputset_identifier = var.refinputset
    pipeline_identifier = var.refpipeline
  })
}
